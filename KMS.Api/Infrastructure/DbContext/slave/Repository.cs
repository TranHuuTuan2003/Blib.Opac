...
namespace KMS.Api.Infrastructure.DbContext.slave
{
    public class Repository<TKeyId, TEntity> : IRepositoryBase<TKeyId, TEntity> where TEntity : BaseEntity<TKeyId>
    {
        ...
        public Repository(UnitOfWorkBlib unitOfWorkBlib, IDateTimeProvider dateTimeProvider, IUserProvider userProvider, AppConfigHelper appConfigHelper)
        {
        ...
        public async Task<List<TEntity>> GetEntitiesAsync(string? columnsQuery, string? whereQuery, string? orderQuery, DbTransaction dbTransaction = null)
        {
            ...
            List<TEntity> result = await _unitOfWorkBlib.Repository.QueryListAsync<TEntity>(sql, null, dbTransaction);
        ...
        public async Task<TEntity> GetEntityByIdAsync(TKeyId id, DbTransaction dbTransaction = null)
        {
            ...
            List<TEntity> result = await _unitOfWorkBlib.Repository.QueryListAsync<TEntity>(sql, param, dbTransaction);
            if (result == null || result.Count == 0)
            {
                return default;
            }
        ...
        public async Task<TEntity> InsertEntityAsync(TEntity entity, DbTransaction dbTransaction = null)
        {
            ...
            int result = await _unitOfWorkBlib.Repository.ExecuteAsync(sql, param, dbTransaction);
            if (result == 0)
            {
            ...
            if (keyIdAuto)
            {
                entity.id = await _unitOfWorkBlib.Repository.QueryFirstAsync<TKeyId>(sql, null);
            }
            else
            {
                entity.id = keyValue;
